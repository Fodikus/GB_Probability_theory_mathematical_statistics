...
formula = 'Задача 2. \n'
formula = formula + 'На входной двери подъезда установлен кодовый замок, содержащий десять кнопок с цифрами от 0 до 9. \n'
formula = formula + 'Код содержит три цифры, которые нужно нажать одновременно. \n'
formula = formula + 'Какова вероятность того, что человек, не знающий код, откроет дверь с первой попытки? \n'
...
formula = formula + 'Из-за того, что кнопки нажимаются одновременно, порядок не важен  \n'
formula = formula + 'm количество благоприятных исходов равна 1, открытию замка \n'
formula = formula + '$\\ m : \\ {1} $ \n'
formula = formula + 'а n количество сочетаний 3-х кнопок из 10. \n'
formula = formula + '$\\ n : \\ C^k_n = С^{3}_{10} $\n'
formula = formula + '\n'
...
# Ответ
result = P * 100
formula = formula + 'Вероятность того, что человек, не знающий код, откроет дверь с первой попытки:\n'
